...
%%maybe we should be more specific here: write down def. of test and training dat a la hastie and tibshirani, define prediction error (many people in econ not familiar! ) and introduce notation for the size of the training set/rel. share of the training set etc. When we vary the 
...
%%%N_{T} is the notation for the number of simulation draws in the training data, we can do another notation

\subsection{Varying $N_{T}$}


%	\includegraphics{bootstrap_mae.pdf} and include a version of this plot %did you try to fit the neural net with fewer observations as well so that it would makes sense to add a plot with the nn? It would also be fine to add a few points of the neural net, because we don't want to fit it for too many values of N
%then write a few sentences about the result, characterize the best method w.r.t. mae for different N_T


\subsection{Variable selection}

%	\includegraphics{ridge_variable_selection.pdf} 

%%%Write something about the figure: describe the procedure, why do we want to do variable selection, why does it not seem feasible in this case: (1) strucutral mdoels not sparse by assumption, (2) parameters possibly correlated in groups/high nonlinearity within groups, parameters jointly important... 
\end{document}
